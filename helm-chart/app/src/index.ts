...
async function handleEdits(req: Request): Promise<Response> {
    ...
    if (contentType.includes("multipart/form-data")) {
        ...
        if (imageFiles.length === 0) {
            return Response.json({ error: "Missing 'image' or 'image[]' field in form data" }, { status: 400 });
        }
...
function stripImageUrlContent(messages: any[]): any[] {
    return messages.map((msg) => {
        ...
        const filtered = msg.content.filter((part: any) => part.type !== "image_url");
...
async function handleChatCompletions(req: Request): Promise<Response> {
    ...
    if (!CHAT_BASE_URL) {
        return Response.json({ error: "CHAT_BASE_URL not configured" }, { status: 500 });
    }
    ...
    if (hadImageContent) {
        ...
        log("🧹 stripped image_url from /chat/completions", {
            messageCount: body.messages.length,
        });
    }

    log("📥 outgoing -> chat /chat/completions", {
        model: body.model,
        messageCount: body.messages?.length,
        stream: body.stream,
    });
    ...
    log(`✅ response <- chat (${chatRes.status})`, {
